...
interface ConnectionPool {
  init(devices: Device[]): void;          // 页面初始化：分组创建连接
  addDevice(device: Device): void;        // 添加设备：复用或新建连接
  removeDevice(machineId: string): void;  // 删除设备：取消订阅、检查清退
  updateDevice(machineId: string, partial: Partial<Device>): void; // 编辑设备
  getConnection(machineId: string): MqttClientLike | null;
  // 主题切换（由 useMqttConnect 调用）
  switchSelection(oldId: string | null, newId: string): void;
}
```
...
- 双向 TLS（clientCert/clientKey）— 不实现
- 密码加密存储 — 当前明文 localStorage
- 自动发现改造 — HTTP API 砍掉后功能待后续独立 PRD 处理
- EMQX HTTP API — 完全移除
- Settings 页面改造
